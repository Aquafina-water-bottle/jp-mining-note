...
export class MobilePopup extends Module {
  ...
  private readonly textEle = document.getElementById(
    'mobile_popup_text'
  ) as HTMLElement | null;
  ...
  private readonly titleCloseEle = document.getElementById(
    'mobile_popup_title_close'
  ) as HTMLElement | null;
  private readonly wrapperEle = document.getElementById(
  ...
  resetSelectionIndicators() {
    const eles = this.wordIndsGroupEle?.querySelectorAll(
      '.card-description__word-indicator'
    );
    if (eles !== undefined) {
  ...
  setupWordIndicator(
    ...
    indEle.onclick = () => {
      ...
      const previousIndLabel = this.ele
        .querySelector('[data-word-indicator-search]')
        ?.getAttribute('data-word-indicator-search');
      ...
      if (
        this.wrapperEle.classList.contains('hidden') ||
  ...
  setupKanjiHover(
    ...
    for (const kanjiEle of kanjiEles) {
      (kanjiEle as HTMLElement).onclick = () => {
        ...
        const previousKanji = this.ele
          .querySelector('[data-kanji-search]')
          ?.getAttribute('data-kanji-search');
