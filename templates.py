...
<script>
  ...
  if (typeof JPMNOpts === 'undefined') {
    var settingsPromise = injectScript('jp-mining-note-options.js');
  }
  settingsPromise.then(() => {
    ...
    var scriptIsLoaded = document.querySelectorAll("script[src='jp-mining-note-options.js']").length > 0;
    if (!scriptIsLoaded) {
      logger.error("Options file not found! Make sure `jp-mining-note-options.js` is placed in the media folder.");
    }
  })
